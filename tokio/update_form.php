...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT id, assistant_content, system_content FROM messages WHERE id = :id");
        $stmt->bindParam(':id', $id, PDO::PARAM_INT);
        $stmt->execute();

        if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $assistantContent = $row['assistant_content'];
            $systemContent = $row['system_content'];
        } else {
...
<div class="container mt-5">
    <h2>Edit Training Message</h2>
    <form action="update_process.php" method="post">
        <input type="hidden" name="id" value="<?php echo htmlspecialchars($id); ?>">
        <div class="form-group">
            <label for="assistant_content">Assistant Content:</label>
            <textarea class="form-control" id="assistant_content" name="assistant_content" rows="3" required><?php echo htmlspecialchars($assistantContent); ?></textarea>
        </div>
        <div class="form-group">
            <label for="system_content">System Content:</label>
            <textarea class="form-control" id="system_content" name="system_content" rows="6" required><?php echo htmlspecialchars($systemContent); ?></textarea>
        </div>
