...
<div class="container mt-5">
    ...
    <h2>Training Messages (active)</h2>
    <table class="table table-bordered table-hover">
        <thead class="thead-dark">
            <tr>
                <th>ID</th>
                <th width=20%>Assistant Content </th>
...
try {
    ...
    $stmt = $pdo->prepare("SELECT id, assistant_content, system_content, status FROM messages WHERE status='active' LIMIT :perPage OFFSET :offset");
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        echo "<td><a href='update_form.php?id=" . $row['id'] . "'>Edit</a> | <a href='delete_process.php?id=" . $row['id'] . "'>Delete</a> | <a href='toggle_status_r.php?id=" . $row['id'] . "&status=" . $toggleStatus . "'>Toggle Status</a></td>";
        echo "</tr>";
    }
        // ดึงจำนวนข้อมูลทั้งหมดเพื่อคำนวณจำนวนหน้า
        $stmt = $pdo->query("SELECT COUNT(*) FROM messages WHERE status='active'");
